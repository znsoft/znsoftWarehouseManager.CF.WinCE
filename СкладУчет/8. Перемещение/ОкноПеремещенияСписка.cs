...
{
    public partial class ОкноПеремещенияСписка : Form
    {
        ...
        public ОкноПеремещенияСписка(ПоследовательностьОкон ПоследовательностьОкон)
        {
            InitializeComponent();
        ...
        private void ОкноПеремещенияСписка_Load(object sender, EventArgs e)
        {
            Информация.Text = "";
        ...
        private void ОкноПеремещенияСписка_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void СканТовара(string СтрокаСкан)
        {
            ...
            if (НайденСкан == null)
            {
                ...
                if (Поиск == null) { Инфо.Ошибка("Товар не найден на полке"); return; }
            ...
            if (НайденСкан == null) { Инфо.Ошибка("Товар не найден на полке"); return; }
        ...
        private void ВвестиКоличествоВручную(ListViewItem ВыбраннаяСтрока, EventArgs e)
        {
            ...
            Point p = GetItemPosition(СписокПеремещения.Handle, ВыбраннаяСтрока.Index);
            p.X += r;
