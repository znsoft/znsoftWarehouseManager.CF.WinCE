...
{
    public partial class ОкноВыбораИзДерева : Form
    {
        private Пакеты Обмен;
        private ПоследовательностьОкон Последовательность;
        private string ТекущийАдрес = null;

        public ОкноВыбораИзДерева(ПоследовательностьОкон ПоследовательностьОкон)
        {
            InitializeComponent();
        ...
        private void ОкноВыбораИзДерева_Load(object sender, EventArgs e)
        {
            ЗаполнитьДерево();
        ...
        private void ОкноВыбораИзДерева_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if ((e.KeyCode == System.Windows.Forms.Keys.D0) || РаботаСоСканером.НажатаЛеваяПодэкраннаяКлавиша(e) || (e.KeyCode == System.Windows.Forms.Keys.Escape))
            {
                e.Handled = true;
                _Назад();
            }
            if ((e.KeyCode == System.Windows.Forms.Keys.D1) || РаботаСоСканером.НажатаПраваяПодэкраннаяКлавиша(e) || (e.KeyCode == System.Windows.Forms.Keys.Enter))
            {
                e.Handled = true;
                _Далее();
            }
